...
int thcCFDInput(int argc, char* argv[], ParameterList &thcPL )
{
  ...
  */
  ...
  PLdump.set<string>("readFileName", "../18061401/flateplate3000", "File name to read from");
   ...
   // fluid input
  ParameterList& PLphys = thcPL.sublist("fluid");
  ...
  //PLphys.set<string>("ipSpace", "simple", "Inner product space: simple, chan, graph",ipSpaceValidator);
   // initialization input but put into fluid
  ...
  if( parse_return == CommandLineProcessor::PARSE_HELP_PRINTED ) {
    // Return error code just to quit program, but don't give error
    return 1;
  };
  ...
  // File can override the above defaults
  updateParametersFromYamlFile(thcInFileName, inoutArg(thcPL));
  ...
  // State the norm
  string lpl_ips = getConst(thcPL).sublist("fluid").get<string>("ipSpace");
